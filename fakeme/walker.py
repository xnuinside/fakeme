...
class Walker(object):
    ...
    ddl_schemas_ignore_list = ['database', 'drop_', 'drop', 'db', 'rename',
                               'create_schemas']
    ...
    def walk(self):
        ...
        if not self.recursive:
            ...
            files = glob.glob(os.path.join(
                self.path_to_dir, '**/*.{}'.format(self.extension)))
            [files.append(file_path) for file_path in glob.glob(os.path.join(
                self.path_to_dir, '*.{}'.format(self.extension)))]
        ...
        with io.open(file_path, "r", encoding=_encoding) as f:
            for line in f.readlines():
                if string in line and not line.startswith("--"):
                    ...
                    if not full:
                        if line.split(string)[1].startswith(
                                " ") or line.split(string)[1].startswith("\n"):
                            operation_key = line.split(" ")[0].lower()
                            if not usages_dict.get(operation_key):
                                usages_dict[operation_key] = {}
                            if not usages_dict[operation_key].get(
                                    file_path):
                                usages_dict[operation_key][
                                    file_path] = []
                            usages_dict[operation_key][
